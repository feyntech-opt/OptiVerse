...
# Objective function
model.obj = Objective(
    expr=sum(model.z[w] for w in range(n_wards)),
    sense=maximize
)

# Constraints
model.budget = Constraint(
    expr=sum(model.a[c, f] + model.n[c, f] for c in range(n_channels) for f in range(n_factors)) + 
         sum(model.k[r, f] for r in range(n_regions) for f in range(n_factors)) <= B
)
...
for w in range(n_wards):
    total_support = sum((model.a[c, f] + model.n[c, f]) * factors_importance[w, f] * alpha[f]
                        for c in range(n_channels) for f in range(n_factors) if wards_to_channels[c, w] == 1) + \
                    sum(model.k[wards_to_regions[w], f] * factors_importance[w, f] * beta[f] for f in range(n_factors))
    model.winning_constraints.add(expr=total_support - 0.5 * voters[w] <= M * (1 - model.z[w]))
    model.winning_constraints.add(expr=total_support >= 0.5 * voters[w] - M * (1 - model.z[w]))

# 2/3 majority constraint
model.majority = Constraint(expr=sum(model.z[w] for w in range(n_wards)) >= 2 * n_wards / 3)
...
csv_output = pd.DataFrame(index=wards_names, columns=factors_names + ['Total Allocation', 'Won'])

for w in range(n_wards):
    for f in range(n_factors):
        total_allocation = sum((model.a[c, f].value + model.n[c, f].value) 
                               for c in range(n_channels) if wards_to_channels[c, w] == 1) + \
                           model.k[wards_to_regions[w], f].value
        csv_output.at[wards_names[w], factors_names[f]] = total_allocation
    csv_output.at[wards_names[w], 'Total Allocation'] = sum(csv_output.loc[wards_names[w], factors_names])
