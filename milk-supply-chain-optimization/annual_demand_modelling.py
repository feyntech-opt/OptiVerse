...
products = {
    "Fresh Milk": {"shelf_life": 7, "time_unit": "daily", "relative_demand": 0.50},
    "UHT Milk": {"shelf_life": 180, "time_unit": "weekly", "relative_demand": 0.10},
    "Butter": {"shelf_life": 90, "time_unit": "weekly", "relative_demand": 0.10},
    "Cheese": {"shelf_life": 120, "time_unit": "weekly", "relative_demand": 0.08},
    "Ghee": {"shelf_life": 365, "time_unit": "monthly", "relative_demand": 0.10},
    "Yogurt": {"shelf_life": 21, "time_unit": "daily", "relative_demand": 0.15},
    "Milk Powder": {"shelf_life": 365, "time_unit": "monthly", "relative_demand": 0.03},
    "Condensed Milk": {"shelf_life": 365, "time_unit": "monthly", "relative_demand": 0.04}
}


def seasonal_factor(day_of_year, product):
    base = 1 + 0.1 * np.sin(2 * np.pi * day_of_year / 365)
    if product in ["Fresh Milk", "UHT Milk", "Yogurt"]:
        ...
        if info['time_unit'] == 'daily':
            ...
            for date in date_range:
                ...
                for _, retail_unit in retail_units.iterrows():
                    demand = np.random.normal(seasonal_demand / NUM_RETAIL_UNITS,
                                              seasonal_demand / NUM_RETAIL_UNITS / 10)
                    demand_data.append({
                        'Date': date,
                        'Product': product,
...
demand_df.to_csv('data/amul_retail_demand_2025_updated.csv', index=False)
