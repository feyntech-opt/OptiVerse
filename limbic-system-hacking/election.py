...
def expected_votes(x, f):
    return 100 + 0.1 * x + 50 * f['caste'] + 30 * f['religion'] + 20 * f['age'] + 40 * f['social_issues'] + 60 * f['target_group_fit']

# Probability of support function (example: depends on ward characteristics)
def probability_support(f):
    return 0.5 + 0.2 * f['caste'] + 0.1 * f['religion'] + 0.1 * f['age'] + 0.2 * f['social_issues'] + 0.3 * f['target_group_fit']
...
# Objective function
model.obj = Objective(
    expr=sum(expected_votes(model.x[i], {k: factors[k][i] for k in factors}) * probability_support({k: factors[k][i] for k in factors}) for i in range(n_wards)),
    sense=maximize
)
...
print("Expected votes:", sum(expected_votes(investment_plan[i], {k: factors[k][i] for k in factors}) * probability_support({k: factors[k][i] for k in factors}) for i in range(n_wards)))
