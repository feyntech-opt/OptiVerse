...
def maximize_engagement_extensive(
    notifications,
    time_slots,
    engagement_levels,
    response_times,
    ...
    for i in notifications:
        if response_times[i] > response_threshold:
            print(
                f"Skipping constraint for {i} due to response time constraint making it infeasible"
            )
            continue
        for t in time_slots:
            if response_times[i] > response_threshold:
                model.Add(x[i, t] == 0)
                print(
                    f"Constraint: Notification {i} cannot be sent at time {t} due to response time"
                )
        ...
        for i in notifications:
            for t in time_slots:
                if activity_levels[t - 1] > activity_threshold:
                    print(
                        f"Adding Activity Level Constraint: Notification {i} cannot be sent at time {t} due to activity level"
                    )
                    model.Add(x[i, t] == 0)
        ...
        for i in notifications:
            for t in time_slots:
                if locations[t - 1] in restricted_areas:
                    print(
                        f"Adding Location Constraint: Notification {i} cannot be sent at time {t} due to location"
                    )
                    model.Add(x[i, t] == 0)
        ...
        for i in notifications:
            for t in time_slots:
                if battery_levels[t - 1] < minimum_battery:
                    print(
                        f"Adding Battery Level Constraint: Notification {i} cannot be sent at time {t} due to low battery level"
                    )
                    model.Add(x[i, t] == 0)
        ...
        for i in notifications:
            for t in time_slots:
                if age < min_age or age > max_age:
                    print(
                        f"Adding Age Constraint: Notification {i} cannot be sent at time {t} due to age restrictions"
                    )
                    model.Add(x[i, t] == 0)
        ...
        for i in notifications:
            for t in time_slots:
                if profile_info["occupation"] not in target_occupations:
                    print(
                        f"Adding Occupation Constraint: Notification {i} cannot be sent at time {t} due to occupation"
                    )
                    model.Add(x[i, t] == 0)
    ...
    if status == cp_model.FEASIBLE or status == cp_model.OPTIMAL:
        ...
        print(
            "No feasible solution found. Adjusting constraints or parameters may be necessary."
        )
        return
...
response_times = {"N1": 5, "N2": 3, "N3": 7, "N4": 2, "N5": 6, "N6": 4}
response_threshold = 6

maximize_engagement_extensive(
    notifications, time_slots, engagement_levels, response_times, response_threshold
)
